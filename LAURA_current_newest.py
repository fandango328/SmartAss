...
config = {
   ...
   # "ELEVENLABS_KEY": ELEVENLABS_KEY,  # Added this
    ...
    "TTS_ENGINE": "alltalk",
    "ALLTALK_HOST": "http://192.168.0.50:7851",  # Keep your current host
    "ALLTALK_VOICE": "voices/Laura.wav",  # Update path to match alltalk_tts directory structure
    "ALLTALK_MODEL": "xtts"  # Simplified model name
}
...
class TTSHandler:
    ...
    def initialize(self):
        ...
        print("Attempting to initialize TTS...")
        try:
            retries = 3
            while retries > 0:
                if self.check_server_ready():
                    ...
                    if self.current_settings:
                        ...
                        return True
                print(f"Server not ready, retrying... ({retries} attempts left)")
                retries -= 1
                time.sleep(2)
            return False
        except Exception as e:
    ...
    def _fetch_server_info(self):
        """Fetch server settings and capabilities"""
        try:
            settings_response = requests.get(
                f"{self.config['ALLTALK_HOST']}/api/currentsettings",
                timeout=5
            )
            voices_response = requests.get(
                f"{self.config['ALLTALK_HOST']}/api/voices",
                timeout=5
            )
            rvc_response = requests.get(
                f"{self.config['ALLTALK_HOST']}/api/rvcvoices",
                timeout=5
            )

            if settings_response.status_code == 200:
                self.current_settings = settings_response.json()
            if voices_response.status_code == 200:
                self.available_voices = voices_response.json().get('voices', [])
            if rvc_response.status_code == 200:
                self.available_rvc_voices = rvc_response.json().get('rvcvoices', [])
                
        except Exception as e:
            print(f"Error fetching server info: {str(e)}")

    def _configure_optimal_settings(self):
        """Configure optimal server settings based on capabilities"""
        if not self.current_settings:
            return

        try:
            # Enable DeepSpeed if available
            if self.current_settings.get('deepspeed_capable'):
                self.set_deepspeed(True)
                print("DeepSpeed enabled")

            # Configure low VRAM mode if needed
            if self.current_settings.get('lowvram_capable'):
                self.set_low_vram(True)
                print("Low VRAM mode enabled")

            # Set voice and model
            if self.config['ALLTALK_VOICE']:
                print(f"Using voice: {self.config['ALLTALK_VOICE']}")
            if self.config['ALLTALK_MODEL']:
                print(f"Using model: {self.config['ALLTALK_MODEL']}")

            # Update additional settings
            if self.config.get('ALLTALK_CONFIG'):
                alltalk_config = self.config['ALLTALK_CONFIG']
                if 'temperature' in alltalk_config:
                    self.current_settings['temperature'] = alltalk_config['temperature']
                if 'repetition_penalty' in alltalk_config:
                    self.current_settings['repetition_penalty'] = alltalk_config['repetition_penalty']
                if 'generation_speed' in alltalk_config:
                    self.current_settings['generation_speed'] = alltalk_config['generation_speed']
                if 'pitch' in alltalk_config:
                    self.current_settings['pitch'] = alltalk_config['pitch']
                if 'streaming_enabled' in alltalk_config:
                    self.current_settings['streaming_enabled'] = alltalk_config['streaming_enabled']
    ...
    def check_server_ready(self):
        ...
        try:
            ...
            ready_response = requests.get(
                f"{self.config['ALLTALK_HOST']}/api/ready",
                timeout=5
            )
            
            health_ok = health_response.status_code == 200
            ready_ok = ready_response.status_code == 200 and ready_response.text == "Ready"
            
            if not health_ok or not ready_ok:
                print(f"Server health check failed: Health - {health_response.status_code}, Ready - {ready_response.status_code}")
    ...
    def _generate_alltalk(self, text):
        ...
        try:
            ...
            # Add optional parameters if available in settings
            if self.config.get('ALLTALK_CONFIG'):
                alltalk_config = self.config['ALLTALK_CONFIG']
                if 'temperature' in alltalk_config:
                    request_data["temperature"] = alltalk_config['temperature']
                if 'repetition_penalty' in alltalk_config:
                    request_data["repetition_penalty"] = alltalk_config['repetition_penalty']
                if 'generation_speed' in alltalk_config:
                    request_data["generation_speed"] = alltalk_config['generation_speed']
                if 'pitch' in alltalk_config:
                    request_data["pitch"] = alltalk_config['pitch']
                if 'streaming_enabled' in alltalk_config:
                    request_data["streaming_enabled"] = alltalk_config['streaming_enabled']
            ...
            print(f"AllTalk generation failed: {str(e)}")
    ...
    def display_server_info(self):
        """Display current server information and capabilities"""
        ...
        if self.current_settings:
            ...
            capabilities = {
                ...
                "Current Model": self.current_settings.get('current_model', 'N/A'),
                "Current Voice": self.current_settings.get('current_voice', 'N/A'),
