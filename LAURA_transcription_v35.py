...
def calculate_cost(tokens: int, is_input: bool) -> float:
    """Calculate cost based on token count and type"""
    if is_input:
        return (tokens / 1000) * 0.003  # $3.00 per million tokens
    else:
        return (tokens / 1000) * 0.015  # $15.00 per million tokens

def count_messages_tokens(messages, include_system=False):
    """Count tokens for messages using Anthropic's token counting endpoint"""
    try:
        message_data = messages.copy()
        ...
        print(f"DEBUG: Token count response: {response}")  # Debug print
        return int(response)  # Convert to integer
    except Exception as e:
        print(f"Error counting tokens: {e}")
        return 0

def update_token_metrics(new_input: int = 0, new_output: int = 0, new_tool: int = 0, new_tool_result: int = 0):
    """Update token counts and costs"""
    global session_tokens
    
    # Update token counts
    session_tokens["input"] += new_input
    session_tokens["output"] += new_output
    session_tokens["tool_use"] += new_tool
    session_tokens["tool_result"] += new_tool_result
    
    # Calculate costs
    if new_input > 0:
        input_cost = calculate_cost(new_input, True)
        session_tokens["costs"]["input"] += input_cost
    
    if new_output > 0:
        output_cost = calculate_cost(new_output, False)
        session_tokens["costs"]["output"] += output_cost
    
    # Update totals
    session_tokens["total"] = (session_tokens["input"] + session_tokens["output"] + 
                             session_tokens["tool_use"] + session_tokens["tool_result"])
    session_tokens["costs"]["total"] = session_tokens["costs"]["input"] + session_tokens["costs"]["output"]

def print_token_metrics(message: str = ""):
    """Print current token usage and costs"""
    print(f"\n{Fore.YELLOW}=== Token Metrics {message} ===")
    print(f"Input tokens: {session_tokens['input']} (${session_tokens['costs']['input']:.4f})")
    print(f"Output tokens: {session_tokens['output']} (${session_tokens['costs']['output']:.4f})")
    print(f"Tool use tokens: {session_tokens['tool_use']}")
    print(f"Tool result tokens: {session_tokens['tool_result']}")
    print(f"Total tokens: {session_tokens['total']}")
    print(f"Total cost: ${session_tokens['costs']['total']:.4f}{Fore.WHITE}")
...
async def generate_response(query):
    global chat_log, last_interaction, last_interaction_check, session_tokens
    ...
    current_query_tokens = count_messages_tokens([{
        "role": "user",
        "content": query,
    }])
    print(f"DEBUG: Current query tokens: {current_query_tokens}")
    
    # Count chat history tokens (excluding current query)
    chat_history_tokens = count_messages_tokens(chat_log[:-1]) if len(chat_log) > 1 else 0
    print(f"DEBUG: Chat history tokens: {chat_history_tokens}")
    
    # Tool choice overhead is added for EACH API call
    tool_choice_overhead = 346
    
    # Initial API call overhead
    total_input_tokens = current_query_tokens + chat_history_tokens + tool_choice_overhead
    print(f"DEBUG: Initial total input tokens (including tool overhead): {total_input_tokens}")
    
    # Update token metrics for input
    update_token_metrics(new_input=total_input_tokens)
    print_token_metrics(
    ...
    try:
        ...
        if response.stop_reason == "tool_use":
            # Add another tool overhead for the final API call with tool results
            tool_choice_overhead = 346
            final_call_tokens = count_messages_tokens(chat_log) + tool_choice_overhead
            update_token_metrics(new_input=tool_choice_overhead)  # Track the additional overhead
        ...
        if response.stop_reason == "tool_use":
            ...
            if tool_results:
                tool_result_tokens = count_messages_tokens([{
                    "role": "user",
                    "content": tool_results
                }])
                update_token_metrics(new_tool_result=tool_result_tokens)
                chat_log.append({
                    "role": "user",
                    "content": tool_results
                })
                print(f"{Fore.YELLOW}Tool result tokens: {tool_result_tokens}{Fore.WHITE}")
            ...
            final_output_tokens = count_messages_tokens([{
                "role": "assistant",
                "content": [block.model_dump() for block in final_response.content]
            }])
            update_token_metrics(new_output=final_output_tokens)
            print(f"{Fore.YELLOW}Final response tokens: {final_output_tokens} | "
                  f"Total session tokens: {session_tokens['total']}{Fore.WHITE}")
            ...
            # Calculate response tokens first
            initial_response_tokens = count_messages_tokens([{
                "role": "assistant",
                "content": [block.model_dump() for block in response.content]
            }])

            update_token_metrics(new_output=initial_response_tokens)
            print(f"{Fore.YELLOW}Response tokens: {initial_response_tokens} | "
                  f"Total session tokens: {session_tokens['total']}{Fore.WHITE}")
...
def estimate_tokens(text):
    """Estimate token count using a simple ratio"""
    # Claude tokenizer averages roughly 3.5 characters per token (approximate)
    if not text:
        return 0
    return len(text) // 3 + 1
...
def count_messages_tokens(messages, include_system=False):
    ...
    try:
        ...
        print(f"DEBUG: Token count response: {response}")
        return response.input_tokens  # Use the input_tokens attribute
    except Exception as e:
        print(f"Error counting tokens: {e}")
        return 0

def print_token_metrics(**kwargs):
    try:
        current_query_tokens = kwargs.get('current_query_tokens', 0)
        chat_history_tokens = kwargs.get('chat_history_tokens', 0)
        response_tokens = kwargs.get('response_tokens', 0)
        label = kwargs.get('label', '')
        
        # Get system tokens
        system_response = anthropic_client.messages.count_tokens(
            messages=[],
            system=SYSTEM_PROMPT,
            model="claude-3-sonnet-20240229"
        )
        system_tokens = system_response
        
        # Rest of the function remains exactly the same
        current_total_input = int(current_query_tokens) + int(system_tokens) + int(chat_history_tokens)
        input_cost = calculate_cost(current_total_input, True)
        cumulative_input_cost = session_tokens["costs"]["input"]
        
        current_output_cost = calculate_cost(response_tokens, False)
        cumulative_output_cost = session_tokens["costs"]["output"]
        
        print(f"\n{Fore.YELLOW}=== Token Metrics {label} ===")
        print(f"Current query Input tokens: {current_query_tokens}")
        print(f"System prompt tokens: {system_tokens}")
        print(f"Input tokens from chat history: {chat_history_tokens}")
        print(f"Total input tokens/cost this query: {current_total_input} (${input_cost:.4f})")
        print(f"Cumulative input token cost this session: (${cumulative_input_cost:.4f})")
        print(f"\nCurrent response tokens: {response_tokens}")
        print(f"Previous responses token total: {session_tokens['output']}")
        print(f"Total response tokens/cost this query: {response_tokens} (${current_output_cost:.4f})")
        print(f"Cumulative response token cost this session: (${cumulative_output_cost:.4f})")
        print(f"{Fore.WHITE}")
    except Exception as e:
        print(f"Error in print_token_metrics: {e}")
