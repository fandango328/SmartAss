...
def get_tools_for_query(query):
    """
    Intelligently detect which tools might be relevant to the query.
    ...
    tool_keywords = {
        "draft_email": ["email", "write", "send", "message"],
        "calendar_query": ["schedule", "event", "calendar", "meeting"],
        "get_current_time": ["time", "date", "today"],
        "create_calendar_event": ["create event", "schedule", "add meeting"],
        "get_location": ["where am i", "location", "address"],
        "read_emails": ["check email", "read emails", "inbox"],
        "manage_tasks": ["task", "todo", "to-do"],
        # Add more mappings
    }
    ...
    for tool_name, keywords in tool_keywords.items():
        if any(keyword in query_lower for keyword in keywords):
            ...
            confidence += 0.3  # Increment confidence
    
    # Adjust confidence calculation
    confidence = min(confidence, 1.0)
    
    needs_tools = len(relevant_tools) > 0
...
async def generate_response(query):
    ...
    try:
        ...
        # Get tool status from token_tracker
        needs_tools, relevant_tools, confidence = token_tracker.get_tools_for_query(query)
        use_tools = token_tracker.tools_are_active()

        print(f"Debug - Tools Status: needs_tools={needs_tools}, use_tools={use_tools}, relevant_tools={relevant_tools}")
        print(f"Debug - Authorization State: {token_tracker.authorization_state}")

        # If tools needed but not authorized, request authorization
        if needs_tools and confidence >= 0.9 and not use_tools:
        ...
        # Add tools if authorized and needed
        if needs_tools and use_tools and relevant_tools:
            api_params["tools"] = relevant_tools
            api_params["tool_choice"] = {"type": "auto"}

        response = anthropic_client.messages.create(**api_params)

        # Rest of your existing tool handling code stays exactly the same
        if response.stop_reason == "tool_use":
            ...
            # Your existing tool execution code stays exactly the same
            tool_results = []
    ...
    while True:
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                if token_tracker.authorization_state == "requested":
