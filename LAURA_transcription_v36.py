...
class TokenTracker:
    def __init__(self, anthropic_client):
        ...
        self.TOOL_OVERHEAD = 346  # Claude 3.7 Sonnet tool overhead
        self._initialize_system_prompt_count()

    def _initialize_system_prompt_count(self) -> None:
        """Count initial system prompt tokens"""
        try:
            if SYSTEM_PROMPT:  # Assuming SYSTEM_PROMPT is globally available
    ...
    def count_message_tokens(self, messages: List[Dict[str, str]], 
                           include_system: bool = False) -> int:
        """
        Count tokens for a list of messages using Anthropic's token counting endpoint
    ...
    def update_query_tokens(self, query: str, chat_history: List[Dict[str, str]], 
                          using_tools: bool = False) -> None:
        """
        Update token counts for a new query
        
        Args:
            query: The user's query text
            chat_history: Previous conversation messages
            using_tools: Whether tools will be used
        """
        try:
            # Count current query tokens
            query_tokens = self.count_message_tokens([{
                "role": "user",
                "content": query
            }])
            ...
            # Add tool overhead if using tools
    ...
    def update_response_tokens(self, response_content: Union[str, List[Dict]], 
                             is_tool_result: bool = False) -> None:
        """
        Update token counts for a response
        
        Args:
            response_content: Response content from Claude
            is_tool_result: Whether this is a tool result
        """
        try:
            # Format response for token counting
            response_message = {
                "role": "assistant",
