...
def load_recent_context(token_limit=None):
    ...
    # Handle token limits first - now using the passed in token_limit
    trimmed_messages = trim_chat_log(all_messages, max_tokens=token_limit)
    
    # Let sanitize_messages_for_api handle role alternation and first user message
    messages = sanitize_messages_for_api(trimmed_messages)
    
    # Only handle ending with assistant message
    if messages and messages[-1]["role"] != "assistant":
        messages = messages[:-1]
        
    # Verify we have at least one exchange
    if len(messages) < 2:
        return []
...
async def process_response_content(content):
    ...
    # Parse mood indicator and get clean message
    mood_match = re.match(r'^\[(.*?)\](.*)', text, re.IGNORECASE)
    if mood_match:
        mood = map_mood(mood_match.group(1))
        message = mood_match.group(2).strip()  # Get just the message part
