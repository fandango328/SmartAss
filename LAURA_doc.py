...
def trim_chat_log(log, max_tokens=None):
    ...
    # STEP 3: Identify complete tool pairs
    complete_pairs = {
    ...
    # STEP 4: Calculate token budget and keep most recent messages that fit
    current_size = 0
    if system_message:
        current_size = token_tracker.count_message_tokens([system_message])
    ...
    for i in range(len(log) - 1, -1, -1):
        message = log[i]
        msg_tokens = token_tracker.count_message_tokens([message])
        
        # Check if adding this message would exceed budget
        if current_size + msg_tokens <= max_tokens:
            result.insert(1 if system_message else 0, message)
            current_size += msg_tokens
        else:
            break
    
    # STEP 5: Final sanitization pass
    result = sanitize_tool_interactions(result)
    
    print(f"Trimmed chat log from {len(log)} to {len(result)} messages")
    return result

def save_to_log_file(message: Dict[str, Any], command_type: Optional[CommandType] = None) -> None:
    """
    Save message with enhanced command type handling
    
    Args:
        message (dict): The message to save
        command_type (CommandType, optional): Type of command being processed
        
    Examples:
        # System state command
        save_to_log_file({
            "role": "user",
            "content": "enable tools"
        }, command_type=CommandType.SYSTEM_STATE)
        
        # Tool usage
        save_to_log_file({
            "role": "user",
            "content": "get current time"
        }, command_type=CommandType.TOOL_USAGE)
    ...
    # Determine log directory based on command type
    if command_type == CommandType.SYSTEM_STATE:
        log_dir = os.path.join(CHAT_LOG_DIR, "debug_log")
        log_file = f"{log_dir}/debug_log_{today}.json"
    else:
        # Both tool usage and regular conversation go to chat log
        log_dir = os.path.join(CHAT_LOG_DIR, "chat_log")
        log_file = f"{log_dir}/chat_log_{today}.json"
    
    os.makedirs(log_dir, exist_ok=True)
    
    # Enhanced log entry with command type
    log_entry = {
        "api_message": {
            "role": message["role"],
        ...
        "metadata": {
            ...
            "command_type": command_type.value if command_type else "conversation",
            "original_message": message
        }
    }
    
    # Read existing log with error handling
    logs = []
...
def load_recent_context(token_limit=None):
    ...
    for i in range(len(all_messages)-2, -1, -2):
        if i < 0:
            break
        ...
        pair_tokens = token_tracker.count_message_tokens(pair)
        
        if current_tokens + pair_tokens > token_limit:
            break
            
        trimmed_messages.insert(0, user_msg)
        trimmed_messages.insert(1, assistant_msg)
        current_tokens += pair_tokens

    print(f"Loaded {len(trimmed_messages)} messages from previous conversation")
...
async def process_response_content(content):
    ...
    print(f"DEBUG - Received content type: {type(content)}")
    print(f"DEBUG - Content preview: {str(content)[:100]}")
    ...
    if isinstance(content, str):
        text = content
        print(f"DEBUG - String content: {text[:100]}")
    else:
        # Handle content blocks from API
        print(f"DEBUG - Non-string content, processing blocks")
        for content_block in content:
            if content_block.type != "text":
                continue
            text = content_block.text
            print(f"DEBUG - Found text block: {text[:100]}")
            break
        else:
            print("DEBUG - No text blocks found")
            await display_manager.update_display('speaking', mood='casual')
            return "No text response found"

    # Parse mood indicator while preserving original content
    mood_match = re.match(r'^\[(.*?)\](.*)', text, re.IGNORECASE)
    if mood_match:
        raw_mood = mood_match.group(1)
        message = mood_match.group(2).strip()
        
        # Only map mood if it's going to be used for display
        mapped_mood = map_mood(raw_mood)
        if mapped_mood:
            await display_manager.update_display('speaking', mood=mapped_mood)
    else:
        # Don't modify the message or force a mood if none was specified
        message = text
    
    # Update chat history with the cleaned message
    ...
    print(f"DEBUG - PROCESS_RESPONSE_CONTENT returning cleaned message: '{message[:50]}...'")
    return message  # Return the cleaned message without mood indicator
...
async def generate_response(query):
    ...
    if is_calibration_command:
        print(f"DEBUG: Direct calibration in generate_response for: '{query_lower}'")
        calibration_success = await run_vad_calibration()
        
        if calibration_success:
            return "Voice detection calibration completed successfully. Your microphone settings have been optimized."
        else:
            return "The voice detection calibration encountered an issue. You might want to try again."    
    ...
    try:
        ...
        if was_command:
            ...
            response_message = command_response.get('message', 'Command processed')
            
            # Update display based on command result
        ...
        # Add document context if files are loaded
        if document_manager and document_manager.files_loaded:
            
            # ADD DEBUG STATEMENTS RIGHT HERE, before the has_images check
            print("\nDEBUG - Document Status:")
            print(f"Files loaded: {list(document_manager.loaded_files.keys())}")
            for filename, file_data in document_manager.loaded_files.items():
                print(f"\nFile: {filename}")
                print(f"Type: {file_data.get('type')}")
                print(f"Content preview: {str(file_data.get('content', ''))[:100]}")
                    
            # Look for image files
            has_images = any(file_data['type'] == 'image' and 'base64' in file_data 
                            for file_data in document_manager.loaded_files.values())
            
            if has_images:
                # Convert the last user message to a multi-content message with images
                content_items = []
                ...
                # Add the user's text query as the final content item
                content_items.append({
                    "type": "text",
                    "text": query
                })
                
                # Replace the last message in chat_log with our image-enhanced message
                chat_log[-1] = {
                    "role": "user",
                    "content": content_items
                }
            
            # Also add text file content to system prompt
            text_context = ""
            for filename, file_data in document_manager.loaded_files.items():
...
async def conversation_mode():
    try:
        ...
        if transcript:
            ...
            # Add both command lists
            load_commands = ["load file", "load files", "load all files", "load my file", "load my files"]
            offload_commands = [
                "offload file", "offload my file", "offload files", "offload my files", 
            ...
            if any(cmd in transcript_lower for cmd in load_commands):
                await display_manager.update_display('tools')
                ...
                if file_loaded_audio:
                    ...
                    await generate_voice("Files have been loaded successfully")
                
                await audio_manager.wait_for_audio_completion()
                await display_manager.update_display('listening')
                return "[CONTINUE] Files have been loaded. What would you like to know about them?"
            ...
            # Check for offload commands
                ...
                file_removed_audio = get_random_audio('file', 'removed')
                if file_removed_audio:
                    await audio_manager.play_audio(file_removed_audio)
                else:
                    await generate_voice("Files have been removed successfully")
                
                await audio_manager.wait_for_audio_completion()
                await display_manager.update_display('listening')
                return "[CONTINUE] Files have been removed. Is there anything else you need?"
...
async def generate_voice(chat):
    ...
    try:
        # REMOVE THIS ENTIRE SECTION - process_response_content already handled the mood
        # mood_pattern = r'^\[(.*?)\](.*)'
...
def sanitize_messages_for_api(messages):
    """Remove extra fields, ensure proper alternation patterns, and validate content for Anthropic API"""
    if not messages:
        return []
    ...
    for msg in messages:
        ...
        if content is None:
            print(f"DEBUG: Skipping message with None content")
            continue
            
        # Check for empty string content
        if isinstance(content, str) and not content.strip():
            print(f"DEBUG: Skipping message with empty string content")
            continue
            
        # Check for empty list content
        if isinstance(content, list) and not content:
            print(f"DEBUG: Skipping message with empty list content")
            continue
            
        # Skip messages that would break alternation
        if prev_role and msg.get("role") == prev_role:
            print(f"DEBUG: Skipping consecutive {msg.get('role')} message to maintain alternation")
            continue
            
        # Keep only the fields API expects
        cleaned_msg = {
            "role": msg.get("role"),
            "content": content
        }
    ...
    if cleaned and cleaned[0].get("role") != "user":
        print("DEBUG: Adding placeholder user message to start conversation")
...
def get_random_audio(category, context=None):
    ...
    if category == "file":
        # Check for specific file load/remove operations
        if context == "loaded":
            specific_file = base_path / "files_loaded.mp3"
            if specific_file.exists():
                print(f"Using specific file for loading: {specific_file}")
                return str(specific_file)
        elif context == "removed" or context == "offloaded":
            specific_file = base_path / "files_removed.mp3"
            if specific_file.exists():
                print(f"Using specific file for removing: {specific_file}")
                return str(specific_file)
    
    # Handle wake word context mapping (existing code)
    ...
    # Direct context folder specified (existing code)
    ...
    # Base category folder (existing code)
    else:
        audio_path = base_path
        print(f"Looking for audio in category folder: {audio_path}")
    
    # Find audio files in the specified path (existing code)
    audio_files = []
...
async def run_main_loop():
    """
    Core interaction loop managing LAURA's conversation flow and command processing.
    
    State Flow:
    1. Idle/Sleep -> Monitors for wake word
    2. Wake -> Acknowledges user and prepares for interaction 
    3. Listening -> Captures and processes user input
    4. Processing -> Handles commands or generates responses
    5. Speaking -> Delivers response with appropriate mood
    6. Returns to Idle/Sleep
    
    Last Updated: 2025-04-01 03:17:51 UTC
    Author: fandango328
    """
    ...
    while True:
        try:
            # ### BEGIN SECTION: IDLE/WAKE STATE HANDLING (2025-03-31 22:29:34) ###
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                await asyncio.sleep(0.1)
                await display_manager.update_display('listening')
                # ### END SECTION: IDLE/WAKE STATE HANDLING ###

                # ### BEGIN SECTION: VOICE INPUT PROCESSING (2025-03-31 22:29:34) ###
                ...
                transcript_lower = transcript.lower().strip()
                # ### END SECTION: VOICE INPUT PROCESSING ###

                # ### BEGIN SECTION: COMMAND DEFINITIONS (2025-03-31 22:29:34) ###
                load_commands = [
                ...
                calibration_commands = [
                    "run voice calibration", "calibrate voice", 
                    "voice calibration", "calibrate voice detection", 
                    "voice detection calibration", "run calibration"
                ]
                # ### END SECTION: COMMAND DEFINITIONS ###

                # ### BEGIN SECTION: CALIBRATION HANDLING (2025-03-31 22:29:34) ###
                is_calibration_command = (
                    ("voice" in transcript_lower or "boys" in transcript_lower) and 
                    any(word in transcript_lower for word in ["calibrat", "detect"])
                )
                
                if is_calibration_command:
                    await display_manager.update_display('tools')
                    print(f"DEBUG: Running voice calibration for: '{transcript_lower}'")
                    
                    calibration_success = await run_vad_calibration()
                    response_message = ("Voice detection calibration completed successfully. "
                                      "Your microphone settings have been optimized." 
                                      if calibration_success else 
                                      "The voice detection calibration encountered an issue. "
                                      "You might want to try again.")
                    
                    await display_manager.update_display('speaking')
                    print(response_message)
                    await generate_voice(response_message)
                    await audio_manager.wait_for_audio_completion()
                    await display_manager.update_display('idle')
                    continue
                # ### END SECTION: CALIBRATION HANDLING ###

                # ### BEGIN SECTION: DOCUMENT LOADING (2025-03-31 22:29:34) ###
                    ...
                    if file_loaded_audio:
                        ...
                        print("WARNING: No file load operation audio found")
                        await generate_voice("Files have been loaded successfully")
                    ...
                    if follow_up:
                        if isinstance(follow_up, str) and follow_up.startswith("[CONTINUE]"):
                            follow_up = follow_up.replace("[CONTINUE]", "").strip()
                        
                        await display_manager.update_display('thinking')
                        res = await generate_response(follow_up)
                        
                        print(f"\nDEBUG PRE-RESPONSE: follow_up = '{follow_up}'")
                        print(f"\nDEBUG POST-RESPONSE: res = '{res}'")
                        print(f"\nDEBUG HOOK CHECK: has_conversation_hook(res) = {has_conversation_hook(res)}")
                        ...
                        # FIX: Handle when no follow-up input is detected
                        await display_manager.update_display('idle')
                    
                    continue
                # ### END SECTION: DOCUMENT LOADING ###

                # ### BEGIN SECTION: DOCUMENT OFFLOADING (2025-03-31 22:29:34) ###
                elif any(cmd in transcript_lower for cmd in offload_commands):
                    await display_manager.update_display('tools')
                    await document_manager.offload_all_files()
                    ...
                    if file_removed_audio:
                        ...
                        print("WARNING: No file removal operation audio found")
                        await generate_voice("Files have been removed successfully")
                    ...
                    continue
                # ### END SECTION: DOCUMENT OFFLOADING ###

                # ### BEGIN SECTION: NORMAL CONVERSATION FLOW (2025-03-31 22:29:34) ###
                try:
                    res = await generate_response(transcript)
                    print(f"\n{Style.BRIGHT}Laura:{Style.NORMAL}")
                    print(f"DEBUG - RESPONSE FROM GENERATE_RESPONSE: '{res}'")
                    print(res)
                    print(f"DEBUG - ABOUT TO CALL GENERATE_VOICE WITH TEXT LENGTH: {len(res)}")
                    await generate_voice(res)
                    ...
                    if isinstance(res, str) and has_conversation_hook(res):
                        ...
                        while has_conversation_hook(res):
                            ...
                            if follow_up:
                                await display_manager.update_display('thinking')
                                try:
                                    if follow_up == "Files have been loaded. What would you like to know about them?":
                                        await generate_voice(follow_up)
                                        await audio_manager.wait_for_audio_completion()
                                        await display_manager.update_display('listening')
                                        continue
                    ...
                    await display_manager.update_display('idle')
                # ### END SECTION: NORMAL CONVERSATION FLOW ###
            
            # ### BEGIN SECTION: TIMEOUT HANDLING (2025-03-31 22:29:34) ###
            else:
                timeout_audio = get_random_audio('timeout')
                if timeout_audio:
                    await audio_manager.play_audio(timeout_audio)
