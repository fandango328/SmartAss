...
async def generate_response(query):
    ...
    try:
        ...
        for msg in sanitize_messages_for_api(chat_log):
...
def sanitize_messages_for_api(messages):
    ...
    # Always include the most recent message if it exists
    ...
    # Process remaining messages with alternation
    prev_role = cleaned[0].get("role") if cleaned else None
    
    for msg in reversed(messages[:-1]):  # Process older messages in reverse order
        ...
        cleaned.insert(0, cleaned_msg)  # Insert at beginning to maintain chronological order
