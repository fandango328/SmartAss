...
# Initialize transcription system based on configuration
try:
...
try:
    ...
    # Continue running even if Google integration fails
...
async def generate_response(query):
    ...
    try:
        ...
        if document_manager and document_manager.files_loaded:
            print(f"DEBUG: Adding loaded documents to context")
            document_context = document_manager.get_loaded_content()
            if document_context:
                # Create enhanced system prompt with document context
                enhanced_system_prompt = f"{SYSTEM_PROMPT}\n\nAvailable documents:\n{document_context}"
                print(f"DEBUG: Documents added to context")
        ...
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                # Check for document commands 
                    ...
                    # Get audio from file_sentences directory
                    file_loaded_audio = get_random_audio('file')
                    if file_loaded_audio:
                        await audio_manager.play_audio(file_loaded_audio)
                    else:
                        print("WARNING: No file load operation audio found in /home/user/LAURA/sounds/file_sentences/")
                    ...
                    # Get audio from file_sentences directory
                    file_removed_audio = get_random_audio('file')
                    if file_removed_audio:
                        await audio_manager.play_audio(file_removed_audio)
                    else:
                        print("WARNING: No file removal operation audio found in /home/user/LAURA/sounds/file_sentences/")
