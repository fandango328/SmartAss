...
def trim_chat_log(log, max_tokens=None):
    ...
    print("\n=== Pre-Sanitization Debug ===")
    print(f"Messages being sent to sanitization: {len(chat_log)}")
    print("\nChecking message pairs:")
    for i in range(len(chat_log)-1):
        print(f"\nPair {i}:")
        print(f"Message {i} role: {chat_log[i].get('role')}")
        print(f"Message {i+1} role: {chat_log[i+1].get('role')}")
        if chat_log[i].get('role') == chat_log[i+1].get('role'):
            print("WARNING: Same roles in sequence!")
...
def save_to_log_file(message: Dict[str, Any], command_type: Optional[CommandType] = None) -> None:
    """
    Save message in Anthropic API-compatible format with timestamp
    
    Args:
        message (dict): The message to save with 'role' and 'content'
        command_type (CommandType, optional): Type of command being processed
    """
    # Don't save system commands
    ...
    log_entry = {
        ...
        "type": command_type.value if command_type else "conversation"
    }
    ...
    # Limit log size - keep only the most recent entries
    max_log_entries = 50  # Adjust this based on your needs
    if len(logs) >= max_log_entries:
        logs = logs[-max_log_entries+1:]  # Keep the most recent entries
...
def sanitize_messages_for_api(messages):
    """Remove extra fields and validate content for Anthropic API"""
    print("\n=== Sanitization Process Debug ===")
    print(f"Starting sanitization of {len(messages)} messages")
    print("Role sequence check:")
    roles = [msg.get('role', 'NO_ROLE') for msg in messages]
    print(' -> '.join(roles[-6:]))  # Show last 6 roles
    ...
    for msg in messages:
        ...
        # Message is from log file, extract from api_message
        api_msg = msg.get("api_message", {})
        if api_msg.get("role") and api_msg.get("content"):
            cleaned.append({
                "role": api_msg["role"],
        ...
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                if not is_system_command:
                    ...
                    save_to_log_file({"role": "user", "content": transcript})
                ...
                if ("voice" in transcript_lower or "boys" in transcript_lower) and any(word in transcript_lower for word in ["calibrat", "detect"]):
                    await display_manager.update_display('tools')
                    calibration_success = await run_vad_calibration()
                    ...
                    save_to_log_file({"role": "assistant", "content": assistant_response})
                    ...
                    # Create assistant response
                    assistant_response = "Files have been loaded successfully. What would you like to know about them?"
                    ...
                    # Create assistant response
                    assistant_response = "Files have been removed successfully. Is there anything else you need?"
