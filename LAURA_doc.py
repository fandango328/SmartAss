...
async def generate_response(query):
    ...
    try:
        ...
        if document_manager and document_manager.files_loaded:
            ...
            # Modify chat_log to include document context
            if document_context:
                system_message = next((msg for msg in chat_log if msg["role"] == "system"), None)
                if system_message:
...
async def run_main_loop():
    ...
    Last Updated: 2025-03-27 22:55:44 UTC
    Author: fandango328
    """
    while True:
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                # Check for document commands
                    ...
                    await display_manager.update_display('tools')
                    await document_manager.load_all_files()
                    await audio_manager.play_audio("files_loaded.mp3")
                    await display_manager.update_display('speaking')
                    continue
                    ...
                    await audio_manager.play_audio("files_removed.mp3")
                    await display_manager.update_display('speaking')
                    continue
