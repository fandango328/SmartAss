...
async def process_response_content(content):
    ...
    if mood_match:
        ...
        # Get the rest of the message - THIS IS THE CRITICAL FIX:
        # Remove just the mood prefix, preserving ALL the remaining text including newlines
    ...
    # IMPORTANT: Return the full message with all paragraphs intact
    return message
        ...
        if document_manager and document_manager.files_loaded:
            ...
            for filename, file_data in document_manager.loaded_files.items():
                if file_data['type'] == 'text':
                    ...
                    text_context += truncated_content
                    text_context += "\n### End File ###\n"
...
async def generate_voice(chat):
    try:
        ...
        # NEW: Preprocess text to handle paragraph breaks with natural punctuation
        # Replace double newlines with a period and space, which creates a natural pause
        chat = chat.replace('\n\n', '. ').replace('\n', ' ')
        
        # Debug print to verify full message is being sent
...
def sanitize_messages_for_api(messages):
    """Remove extra fields and ensure proper alternation patterns for Anthropic API"""
    if not messages:
        return []
