...
def load_recent_context(token_limit=None):
    ...
    # Handle token limits first
    trimmed_messages = trim_chat_log(all_messages, max_tokens=CHAT_LOG_RECOVERY_TOKENS)
...
async def run_main_loop():
    ...
    Special Handlers:
    - Document Management: load/offload files
    - Voice Calibration: system audio optimization
    - Conversation Flow: maintains natural dialogue
    - Error Recovery: graceful handling of failures
    
    Last Updated: 2025-03-28 22:21:37 UTC
    Author: fandango328
    """
    while True:
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                detected_model = await wake_word()  # Now returns model name
                
                if not detected_model:
                    await asyncio.sleep(0.1)
                    continue
                
                # First handle display state change
                if display_manager.current_state == 'sleep':
                    await display_manager.update_display('wake')
                    await asyncio.sleep(0.1)
                
                # Then handle audio after display change
                ...
                # Process transcript to lowercase immediately
                ...
                calibration_commands = [
                    "run voice calibration", "calibrate voice", "voice calibration",
                    "calibrate voice detection", "voice detection calibration", 
                    "run calibration"
                ]

                # Check for calibration command - more robust pattern matching for transcription errors
                is_calibration_command = (
                    # Look for "voice" OR common mistranscription "boys"
                    ("voice" in transcript_lower or "boys" in transcript_lower) and 
                    # Look for variations of calibration or detection terms
                    any(word in transcript_lower for word in ["calibrat", "detect"])
                )
                print(f"DEBUG: Checking calibration: '{transcript_lower}' -> {is_calibration_command}")

                if is_calibration_command:
                    await display_manager.update_display('tools')
                    print(f"DEBUG: Directly running voice calibration for: '{transcript_lower}'")
                    
                    calibration_success = await run_vad_calibration()
                    
                    if calibration_success:
                        response_message = "Voice detection calibration completed successfully. Your microphone settings have been optimized."
                    else:
                        response_message = "The voice detection calibration encountered an issue. You might want to try again."
                ...
                # Modified document loading section in run_main_loop()
                    ...
                    # Wait for audio completion before re-engaging microphone
                    await audio_manager.wait_for_audio_completion()
                    await asyncio.sleep(0.3)  # Small pause for natural feel
                    
                    # Re-engage listening mode instead of returning to idle
                    await display_manager.update_display('listening')
                    
                    # Try to get follow-up input
                    follow_up = await conversation_mode()
                    if follow_up:
                        # Process the follow-up input normally
                        ...
                        # Continue with conversation flow if needed
                        if isinstance(res, str) and has_conversation_hook(res):
                            await audio_manager.wait_for_audio_completion()
                            await asyncio.sleep(0.3)
                            await display_manager.update_display('listening')
                            # Let normal conversation flow take over
                        else:
                            await audio_manager.wait_for_audio_completion()
                            await display_manager.update_display('listening')  # Changed from 'idle' to 'listening'
                    
                    continue  # Skip to next iteration of main loop

                # Apply identical changes to the file offload section
                elif any(cmd in transcript_lower for cmd in offload_commands):
                    await display_manager.update_display('tools')
                    await document_manager.offload_all_files()
                    
                    # Specify 'removed' context
                    ...
                    if follow_up:
                        ...
                        # No follow-up detected, return to idle
                        await display_manager.update_display('idle')
                        
                    continue  # Skip to next iteration of main loop

                # Normal API response flow
                try:
                    res = await generate_response(transcript)
                    print(f"\n{Style.BRIGHT}Laura:{Style.NORMAL}")
                    print(f"DEBUG - RESPONSE FROM GENERATE_RESPONSE: '{res}'")

                    # The mood has already been processed in process_response_content
                    # Remove this duplicate mood extraction
                    ...
                    print(res)  # Print the response for logging
