...
async def process_response_content(content):
    ...
    Ensures proper text cleanup for voice and maintains user->assistant pairs in chat_log.
    """
    print(f"DEBUG - [2025-04-03 04:01:22] PROCESS_RESPONSE_CONTENT starting")
    ...
    # Step 4: VERIFICATION - Check if we can properly append to chat_log
    if not chat_log:
        print(f"DEBUG - [2025-04-03 04:01:22] Empty chat_log, skipping append")
        return message    # Return for voice but don't write to chat_log
        
    if chat_log[-1]["role"] != "user":
        print(f"DEBUG - [2025-04-03 04:01:22] Last message was {chat_log[-1]['role']}, not 'user'")
        print(f"DEBUG - Cannot append assistant response, would break pairing")
        return message    # Return for voice but don't write to chat_log
    
    # Step 5: Verification passed - safe to append
    assistant_message = {"role": "assistant", "content": message}
    chat_log.append(assistant_message)
    
    print(f"DEBUG - [2025-04-03 04:01:22] Successfully appended assistant response")
    print(f"DEBUG - Chat_log pair complete: user -> assistant")
...
def sanitize_messages_for_api(chat_log):
    """
    Takes chat_log and ensures proper alternating pairs for API:
    [
    ...
    """
    sanitized = []           # Initialize empty array to store our clean message pairs
    last_role = None        # Track previous role to prevent duplicates

    for msg in chat_log:    # Loop through each message in the input chat history
        # Check if message has valid role (user/assistant) and has content
        if msg["role"] in ["user", "assistant"] and msg["content"]:
            # Only add if role is different from previous (prevents duplicate roles)
            if msg["role"] != last_role:
                # Create clean message object with just role and content
                sanitized.append({
                    "role": msg["role"],      # Copy role from original message
                    "content": msg["content"]  # Copy content from original message
                })
                last_role = msg["role"]       # Update tracker with current role

    return sanitized        # Return the clean array of alternating messages
