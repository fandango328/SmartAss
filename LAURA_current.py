    ...
    try:
        ...
        if transcript:  # here's number 2
            ...
            if tool_enable_commands or tool_disable_commands:
                # Use token_tracker to handle the command
                was_command, command_response = token_tracker.handle_tool_command(transcript)
                if was_command:
                    status_type = command_response.get('state', '').lower()
                    if status_type in ['enabled', 'disabled']:
                        ...
                        # Always return to listening state after tool command
                        await display_manager.update_display('listening')
                        return "[CONTINUE]"
            ...
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                if tool_enable_commands or tool_disable_commands:
                    ...
                    if response == "[CONTINUE]":
                        # Handle both enable and disable the same way - stay in listening mode
                        await audio_manager.wait_for_audio_completion()
                        await display_manager.update_display('listening')
