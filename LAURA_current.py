        ...
        if document_manager and document_manager.files_loaded:
            ...
            for filename, file_data in document_manager.loaded_files.items():
                ...
                if (isinstance(file_data, dict) and 
                    ...
                    print(f"Added image {filename} to system prompt")
                    
                elif file_data.get('type') == 'text':
                    # Add text content
                    text_context += f"\n### File: {filename} ###\n"
                    text_context += file_data['content']
                    text_context += "\n### End File ###\n"
            
            # Construct enhanced system prompt with both images and text
            if content_items:
                # If we have images, create a content list for system prompt
                enhanced_system_prompt = {
                    "role": "system",
                    "content": [
                        *content_items,  # Add all image blocks first
                        {
                            "type": "text",
                ...
                # If only text, keep it as a simple string
                enhanced_system_prompt = f"{SYSTEM_PROMPT}\n\nAvailable documents:\n{text_context}"

        # Prepare API call
        api_params = {
            "model": ANTHROPIC_MODEL,
            "system": enhanced_system_prompt,  # Now contains images if any
            "messages": chat_log,              # Only contains text interactions
            "max_tokens": 1024,
            "temperature": 0.8
        }
