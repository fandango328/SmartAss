...
async def generate_response(query):
    ...
    # If we get here, it's not a system command, so add to chat_log
    user_message = {
        "role": "user",
        "content": query
    }
        ...
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                if ("voice" in transcript_lower or "boys" in transcript_lower) and any(word in transcript_lower for word in ["calibrat", "detect"]):
                    ...
                    # Inside your conversation mode or run_main_loop
                    follow_up = await conversation_mode()
                    if follow_up == "[CONTINUE]":  # <-- ADD THIS SPECIAL CHECK
                        # This is a command that was already handled
                        continue  # Go to next loop iteration, staying in listening mode
                    elif follow_up:
                        
                        # CRITICAL: Make sure we handle the new user input correctly
                        if not chat_log:
                            # First message in an empty chat log
                            print("DEBUG - First message in conversation")
                            chat_log.append({"role": "user", "content": follow_up})
                        elif chat_log[-1]["role"] == "assistant":
                            # Normal flow: assistant followed by user
                            print("DEBUG - Normal flow: Adding user message after assistant")
                            chat_log.append({"role": "user", "content": follow_up})
                        elif chat_log[-1]["role"] == "user":
                            # Two user messages in a row - replace the last one to maintain alternation
                        ...
                        # Debug the chat log before API call
