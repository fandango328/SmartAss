...
async def conversation_mode():
    try:
        ...
        if transcript:  # here's number 2
            ...
            load_commands = ["load file", "load files", "load all files", "load my file", "load of my file", "load my files"]
            offload_commands = [
        ...
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                if tool_enable_commands or tool_disable_commands:
                    # Process tool commands via generate_response but don't try to speak the returned value
                    response = await generate_response(transcript)
                    
                    # FIX: Don't try to generate voice for control signals like [CONTINUE]
                    if response == "[CONTINUE]":
                        # Just wait for the tool command audio to complete
                        await audio_manager.wait_for_audio_completion()
                        await display_manager.update_display('listening')
                        
                        # Add this block to immediately listen for the next command
                        follow_up = await conversation_mode()
                        if follow_up:  # If we get input, process it
                            await display_manager.update_display('thinking')
                            res = await generate_response(follow_up)
                            print(f"\n{Style.BRIGHT}Response:{Style.NORMAL}")
                            print(res)
                            
                            if res == "[CONTINUE]":  # Handle another control signal properly
                                # For nested [CONTINUE], we need to handle display state here
                                await audio_manager.wait_for_audio_completion()
                                
                                # First check if we're enabling or disabling tools
                                follow_up_lower = follow_up.lower().strip()
                                tool_disabling = any(cmd.lower() in follow_up_lower 
                                                     for cmd in SYSTEM_STATE_COMMANDS["tool"]["disable"])
                                
                                if tool_disabling:
                                    # After disabling tools, go back to idle state
                                    await display_manager.update_display('idle')
                                else:
                                    # Otherwise stay in listening mode
                                    await display_manager.update_display('listening')
                                    
                                    # Optionally add another listener here if needed
                                    second_follow_up = await conversation_mode()
                                    if second_follow_up:
                            ...
                            await audio_manager.wait_for_audio_completion()
                            await display_manager.update_display('idle')
                        else:  # No input during the follow-up period
                            await display_manager.update_display('idle')
                        continue
                    ...
                    await display_manager.update_display('tools')
                    await document_manager.offload_all_files() #line no 4800
