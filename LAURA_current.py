...
async def generate_response(query):
    ...
    try:
        ...
        # INSERT THE DEBUG CODE RIGHT HERE, before making the API call
        print("\nDEBUG - Final API request state:")
        print(f"Chat log message count: {len(chat_log)}")
        print("Last 2 messages roles:")
        for i in range(min(2, len(chat_log))):
            print(f"  - Message {len(chat_log)-i-1}: {chat_log[-i-1]['role']}")
        print(f"API messages count: {len(api_params['messages'])}")
        print("First and last API message roles:")
        if api_params['messages']:
            print(f"  - First: {api_params['messages'][0]['role']}")
            print(f"  - Last: {api_params['messages'][-1]['role']}")
...
async def conversation_mode():
    try:
        ...
        if transcript:  # here's number 2
            ...
            # After all command checks, right before returning the transcript
            print(f"\n{Style.BRIGHT}You said:{Style.NORMAL} {transcript}\n")
            # SIMPLIFIED APPROACH: Always save to log file immediately, with no conditions
            user_message = {"role": "user", "content": transcript}
            try:
                save_to_log_file(user_message)
                print(f"\n**** DIRECT SAVE TO LOG FILE: {transcript[:50]}... ****\n")
            except Exception as e:
                print(f"\n**** ERROR SAVING TO LOG FILE: {str(e)} ****\n")
...
async def run_main_loop():   #line no 4518
    """
    ...
    while True:
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                if ("voice" in transcript_lower or "boys" in transcript_lower) and any(word in transcript_lower for word in ["calibrat", "detect"]):
                ...
                elif any(cmd in transcript_lower for cmd in load_commands): #line 4681
                    await display_manager.update_display('tools')
                    await document_manager.load_all_files(clear_existing=False)
                    ...
                    if follow_up == "[CONTINUE]":
                        ...
                        user_message = {"role": "user", "content": follow_up}
                        #line 4700
                        # Always save to log file first
                        save_to_log_file(user_message)   #<--- see i put this here at line 4702
                        
                        # Then handle in-memory chat log
                        if not chat_log:
                            print("DEBUG - First message in conversation")
                            chat_log.append({"role": "user", "content": follow_up}) #line 4707
                    ...
                    if file_removed_audio:
                        ...
                        await generate_voice("Files removed successfully")
                        ...
                        if isinstance(res, str) and has_conversation_hook(res):
                            ...
                            while has_conversation_hook(res):
                                ...
                                if follow_up == "[CONTINUE]":
                                    ...
                                    timeout_audio = get_random_audio("timeout")   #line number 4800
                                    if timeout_audio:
                                        await audio_manager.play_audio(timeout_audio)
                                    else:
                                        await generate_voice("No input detected. Feel free to ask for assistance when needed")
