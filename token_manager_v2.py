...
class TokenManager:
    ...
    def count_message_tokens(self, messages_or_content):
        """
        Count tokens in messages using Anthropic's API.
        Handles both full messages format and raw content.
        
        Args:
            messages_or_content: Full messages array or raw content
            
        Returns:
            int: Token count, or 0 if counting fails
        """
        try:
            # Prepare properly formatted messages array
            if isinstance(messages_or_content, list) and len(messages_or_content) > 0 and isinstance(messages_or_content[0], dict) and "role" in messages_or_content[0]:
                # Already formatted as [{"role": "...", "content": "..."}]
                formatted_messages = messages_or_content
                
            elif hasattr(messages_or_content, "content"):
                # Handle API response object
                content_text = ""
                if isinstance(messages_or_content.content, list):
                    for block in messages_or_content.content:
                        if hasattr(block, "text"):
                            content_text += block.text
                else:
                    content_text = str(messages_or_content.content)
                    
                formatted_messages = [{"role": "assistant", "content": content_text}]
                
            else:
                # Handle raw content (string or other)
                content_text = str(messages_or_content)
                formatted_messages = [{"role": "user", "content": content_text}]
            
            # Call Anthropic's token counting API with proper format
            count_result = self.anthropic_client.messages.count_tokens(
                model=self.query_model,
                messages=formatted_messages
            )
            
            # Check actual response format (could be tokens or token_count)
            if hasattr(count_result, "tokens"):
                return count_result.tokens
            elif hasattr(count_result, "token_count"):
                return count_result.token_count
            else:
                # Last resort - extract from raw response dict
                result_dict = count_result.__dict__ if hasattr(count_result, "__dict__") else {}
                if "tokens" in result_dict:
                    return result_dict["tokens"]
                elif "token_count" in result_dict:
                    return result_dict["token_count"]
                
                # Truly desperate measures - use string length estimation
                print("Warning: Unable to extract token count from API response")
                content_length = 0
                for msg in formatted_messages:
                    content_length += len(str(msg.get("content", "")))
                return content_length // 4  # Rough approximation
            
        except Exception as e:
            print(f"Token counting error: {e}")
            # Fallback estimation (4 chars ≈ 1 token)
            if isinstance(messages_or_content, str):
                return len(messages_or_content) // 4
    ...
    def update_session_costs(self, input_tokens: int, output_tokens: int, 
        ...
        try:
            ...
            # Always return a tuple of two Decimal values
    ...
    def calculate_token_cost(self, model: str, token_type: str, token_count: int) -> Decimal:
        ...
        Returns:
            Decimal cost for the tokens, or Decimal('0') if calculation fails
        """
        try:
            if token_count < 0:
            ...
            if not isinstance(self.MODEL_COSTS, dict):
                print(f"Warning: MODEL_COSTS is not a dictionary")
                return Decimal('0')
                
            if model not in self.MODEL_COSTS:
                print(f"Warning: Unknown model '{model}', using zero cost")
                return Decimal('0')
                
            if token_type not in self.MODEL_COSTS[model]:
                print(f"Warning: Unknown token type '{token_type}' for model '{model}', using zero cost")
                return Decimal('0')
