...
class TokenManager:
    ...
    def __init__(self, anthropic_client):
        ...
        The anthropic_client parameter name is specifically chosen to maintain consistency
        with LAURA_v5.py's instantiation: 
        ...
        Args:
            anthropic_client: Anthropic API client instance for API interactions
                This matches the external variable name for clarity in tracing usage

        Raises:
            ValueError: If ANTHROPIC_MODEL from config is not in MODEL_COSTS
            TypeError: If anthropic_client is None
        """
        # Validate required client instance
        ...
        # Store client reference with consistent naming
        self.anthropic_client = anthropic_client  # Maintains naming alignment with instantiation

        # Model Configuration - Using config settings
        self.query_model = ANTHROPIC_MODEL  # Default model for standard queries
        self.tool_model = "claude-3-5-sonnet-20241022"  # Specific model for tool operations
        
        # Session Tracking - For token and interaction monitoring
        self.total_input_tokens = 0
        self.total_output_tokens = 0
        self.session_start_time = datetime.now()
        self.last_interaction_time = None
        
        # Cost Tracking - Using Decimal for precision in financial calculations
        self.total_tool_definition_costs = Decimal('0.00')
        self.total_tool_usage_costs = Decimal('0.00')
        self.total_query_costs = Decimal('0.00')
        
        # Tool State Management - Binary state system
        self.tools_enabled = False  # Simple enabled/disabled state
        self.consecutive_non_tool_queries = 0  # Counter for auto-disable feature
        self.tool_disable_threshold = 3  # Auto-disable after this many non-tool queries
        self.tools_used_in_session = False  # Tracks if tools were used in current session
    ...
    def handle_tool_command(self, query: str) -> Tuple[bool, Optional[str]]:
        ...
        if has_enable_command and has_disable_command:
            ...
            # Find last position of any enable or disable command
            last_enable_pos = -1
            last_disable_pos = -1
            
            for phrase in self.TOOL_ENABLE_PHRASES:
                pos = query_lower.rfind(phrase)  # Find last occurrence of each enable phrase
                last_enable_pos = max(last_enable_pos, pos)
                
            for phrase in self.TOOL_DISABLE_PHRASES:
                pos = query_lower.rfind(phrase)  # Find last occurrence of each disable phrase
                last_disable_pos = max(last_disable_pos, pos)
