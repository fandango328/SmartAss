...
async def generate_response(query):
    ...
    try:
        ...
        # Analyze query for potential tool use
        needs_tools, relevant_tools, confidence = get_tools_for_query(query)
        
        # AUTHORIZATION FLOW:
        # If high confidence but not yet authorized, request authorization
        if needs_tools and confidence >= 0.9 and not token_tracker.tools_are_active():
            # Request user confirmation before using tools
            token_tracker.request_authorization()
            return f"I'd like to use tools to help with this request. Would you like me to do that?"
        
        # Determine whether to include tools in API call based on authorization state
        ...
        response = anthropic_client.messages.create(
            ...
            tool_choice={"type": "auto"} if use_tools else None
        )
        ...
        if response.stop_reason == "tool_use":
            ...
            for content_block in response.content:
                if content_block.type == "tool_use":
                    ...
                    try:
                        ...
                        # Record that a tool was successfully used
            ...
            if tool_results:
                ...
                # After tool use, prepare to ask about continuation
                token_tracker.request_continuation()
            ...
            # If we're in tool mode but didn't use tools, consider asking about continuation
...
async def main():
    ...
    try:
        ...
        print(f"✓ Model: {token_tracker.model}")
        print(f"✓ Tool Status: {token_tracker.authorization_state}")
    ...
    while True:
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                # Check for tool authorization confirmation
                ...
                if token_tracker.authorization_requested():
                    # Process confirmation response
                    was_confirmation, is_affirmative, message = token_tracker.process_confirmation(transcript)
        
                    if was_confirmation:
                        if is_affirmative:
                            # User confirmed tool usage
                            await display_manager.update_display('speaking')
                            confirm_msg = "Tools enabled. I'll use them to assist you with your request."
                            print(confirm_msg)
                            await generate_voice(confirm_msg)
                            await audio_manager.wait_for_audio_completion()
                
                            # Now process the original query with tools enabled
                            await display_manager.update_display('thinking')
                            res = await generate_response(chat_log[-1]["content"])  # Use the previous query
                        else:
                            # User rejected tool usage
                            await display_manager.update_display('speaking')
                            reject_msg = "I'll proceed without using tools."
                            print(reject_msg)
                            await generate_voice(reject_msg)
                            await audio_manager.wait_for_audio_completion()
                
                            # Process without tools
                            await display_manager.update_display('thinking')
                            res = await generate_response(chat_log[-1]["content"])
                    else:
                        # Not a confirmation response, treat as new query
