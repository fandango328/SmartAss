...
class TokenTracker:
    ...
    def update_query_tokens(self, query, chat_history, using_tools):
        """Count tokens for the query message and update session history"""
        try:
            messages = chat_history + [{"role": "user", "content": query}]
            count_request = {
                "model": ANTHROPIC_MODEL,  # Using the global model constant
                "messages": messages
            }
            if using_tools:
            ...
            # Log the count
            self._log_count(token_count, "query")
            return token_count
    ...
    def update_response_tokens(self, response_blocks, is_tool_result=False):
        """Count tokens for the response messages and update session history"""
        try:
            messages = [{"role": "assistant", "content": block["content"]} for block in response_blocks]
            count_request = {
                "model": ANTHROPIC_MODEL,  # Using the global model constant
                "messages": messages
            }
            ...
            # Log the count
            self._log_count(token_count, "response" if not is_tool_result else "tool_result")
            return token_count
    ...
    def _log_count(self, count, message_type):
        """Internal method to log token counts"""
    ...
    def get_session_summary(self):
        ...
        if not self.session_history:
            return {"total_input_tokens": 0, "message_count": 0}
            
        return {
            "total_input_tokens": sum(entry["input_tokens"] for entry in self.session_history),
            "message_count": len(self.session_history)
        }
    
    def print_metrics(self, context):
        """Print or log session metrics for debugging purposes"""
        summary = self.get_session_summary()
        print(f"=== Token Metrics {context} ===")
        print(f"Input Tokens: {summary['total_input_tokens']}")
        print(f"Message Count: {summary['message_count']}")
        print(f"Session History: {self.session_history}")
...
async def generate_response(query):
    ...
    # Record the current time for interaction tracking
    now = datetime.now()    
    last_interaction = now
    last_interaction_check = now
    
    # Check if the conversation has ended (based on a timeout) and reset if needed
    ...
    try:
        # Analyze the query for tool requirements using the sophisticated analysis function
        needs_tools, relevant_tools, confidence = get_tools_for_query(query)
        
        # Debug output for tool detection
        if needs_tools:
            print(f"DEBUG: Detected tools {relevant_tools} with confidence {confidence:.2f}")
        
        # Update the token tracker with the current query and its context
        token_tracker.update_query_tokens(
            query=query,
            chat_history=chat_log[:-1],
            using_tools=needs_tools
        )
        print("DEBUG: Token tracking updated for query")
    
        # Only include tools in the API request if the confidence is above the threshold
        if needs_tools and confidence >= 0.9:
            ...
            # If no tools are needed or confidence is below the threshold, proceed normally
            response = anthropic_client.messages.create(
                model=ANTHROPIC_MODEL,
                system=SYSTEM_PROMPT,
                messages=chat_log,
        ...
        # Handle the case where the response indicates that a tool should be used
        if response.stop_reason == "tool_use":
            # Update the token tracker for tool usage
            token_tracker.update_query_tokens(
                query="",
            ...
            # Play a tool-related audio notification, if available
            ...
            # Add the assistant's tool usage response to the chat log
            chat_log.append({
                "role": "assistant",
            ...
            if tool_results:
                ...
                # Make final API call with tool results - REMOVED tools and tool_choice parameters
...
async def process_response_content(content):
    ...
    1. Extract text content from response blocks
    2. Parse mood indicators from response
    3. Update display state and mood
    4. Maintain chat history
    
    Args:
        content: List of content blocks from Anthropic API response
        
    Returns:
        str: Processed message text with mood indicators removed
        
    State Effects:
        - Updates display_manager state
        - Modifies global chat_log
    """
    global chat_log  # Explicitly declare global usage
    ...
    for content_block in content:
        # Verify content block type
        if content_block.type != "text":
            continue
            
        text = content_block.text
        # Parse mood indicator if present
        # Format: [mood]message where mood is one of the supported moods
        mood_match = re.match(
            r'^\[(neutral|happy|confused|disappointed|annoyed|surprised)\]', 
            text, 
            re.IGNORECASE
        )

        if mood_match:
            # Extract mood and clean message
            mood = mood_match.group(1).lower()
            message = text[mood_match.end():].strip()
            
            # Update display with detected mood
            ...
            # Handle response without mood indicator
            await display_manager.update_display('speaking', mood='neutral')
            chat_log.append({"role": "assistant", "content": text})
            return text

    # Fallback for no valid text content
    await display_manager.update_display('speaking', mood='neutral')
...
async def main():
    ...
    Handles initialization of all core components, task management, and cleanup.
    
    Global Components:
    - remote_transcriber: Handles remote speech-to-text processing
    - display_manager: Controls the visual interface
    - transcriber: Handles local speech-to-text processing
    - token_tracker: Manages Anthropic API token usage tracking
    """
    # Declare globals that will be modified within this function
    global remote_transcriber, display_manager, transcriber, token_tracker  
    tasks = []  
    
    try:
        # INITIALIZATION PHASE
        # Verify token tracking system is ready before initialization
        verify_token_tracker_setup()

        # Debugging prints
        print(f"{Fore.CYAN}Initializing token tracking system...{Fore.WHITE}")
        token_tracker = TokenTracker(anthropic_client) 
        print(f"Token tracking system initialized with model: {ANTHROPIC_MODEL}")

        # Debug output to verify available token tracking methods
        # This helps diagnose if all expected functionality is present
        # print(f"Available token tracking methods: {dir(token_tracker)}")

        # VERIFICATION PHASE
        try:
            # Comprehensive token tracker verification
            print(f"\n{Fore.CYAN}=== Token Tracking System Verification ===")
            print(f"Token Tracker Status: {'Initialized' if token_tracker else 'Failed'}")
            # Verify system prompt token counting is working
            ...
            print(f"{Fore.RED}Token tracking system verification failed: {e}{Fore.WHITE}")
...
async def run_main_loop():
    """
    Main interaction loop handling the voice assistant's core conversation flow.
    
    State Flow:
    1. Wake Detection (sleep/idle -> wake -> listening)
    2. Voice Input Processing (listening -> thinking)
    3. Response Generation (thinking -> speaking)
    4. Follow-up Handling (speaking -> listening/idle)
    
    Token Tracking:
    - Token usage is tracked in generate_response() function
    - Each query and response is monitored for API limits
    """
    while True:
        try:
            ...
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                # Update display and play wake confirmation
                if display_manager.current_state == 'sleep':
                    await display_manager.update_display('wake')
                
                # Handle wake word audio feedback
                wake_audio = get_random_audio('wake')
                if wake_audio:
                    await audio_manager.play_audio(wake_audio)
                else:
                    await generate_voice("Just so you know, wake specific audio is unavailable at this time") 
                
                # Prepare for voice input
                await audio_manager.wait_for_audio_completion()
                await asyncio.sleep(0.3)  # Buffer for clean audio transition
            ...
            if transcript:
                ...
                # PHASE 3: INITIAL RESPONSE GENERATION
                # -------------------------------------------------------------
                # This calls the Anthropic API via generate_response()
                # Token tracking occurs within generate_response()
                res = await generate_response(transcript)
                print(f"\n{Style.BRIGHT}Laura:{Style.NORMAL}")
                
                # Process response mood and update display
                ...
                # PHASE 4: FOLLOW-UP CONVERSATION HANDLING
                # -------------------------------------------------------------
                if isinstance(res, str) and "?" in res:
                    await audio_manager.wait_for_audio_completion()
                    await asyncio.sleep(0.3)  # Buffer before listening
                    await display_manager.update_display('listening')
                    
                    # Continue conversation while questions are asked
                    while "?" in res:
                        follow_up = await conversation_mode()
                        if follow_up:
                            # Process follow-up response
                            await display_manager.update_display('thinking')
                            # Second Anthropic API call via generate_response()
                            res = await generate_response(follow_up)
                            print(f"\n{Style.BRIGHT}Response:{Style.NORMAL}")
                            
                            # Handle response mood
                            mood_match = re.match(r'\[(.*?)\]', res)
                            if mood_match:
                                mood, message = mood_match.groups()
                                await display_manager.update_display('speaking', mood=mood.lower())
