...
import base64
import struct
import random  # Remove duplicate
import asyncio
import textwrap
import threading
import traceback
...
async def generate_response(query):
    ...
    # Start token tracking session and record interaction time
    now = datetime.now()    
    last_interaction = now
    last_interaction_check = now
    token_tracker.start_interaction()  # NEW: Start tracking this interaction
    ...
    try:
        ...
        # Track initial query before API call
        ...
        # API call with token tracking wrapper
        response = await token_tracker.execute_api_call(
            lambda: anthropic_client.messages.create(
                ...
                max_tokens=1024,
                temperature=0.7,
                tools=TOOLS if (needs_tools and confidence >= 0.9) else None,
                tool_choice={"type": "auto"} if (needs_tools and confidence >= 0.9) else None
            )
        )
        ...
        if response.stop_reason == "tool_use":
            ...
            tool_session = token_tracker.start_tool_session()
            
            print("DEBUG: Tool use detected!")
            await display_manager.update_display('tools')
            
            # Track tool overhead
            await token_tracker.track_tool_overhead(
                tool_count=len([b for b in response.content if b.type == "tool_use"])
            )
            ...
            if tool_results:
                # Track tool results before final API call
                await token_tracker.track_tool_results(tool_results)
                
                final_response = await token_tracker.execute_api_call(
                    lambda: anthropic_client.messages.create(
                        model=ANTHROPIC_MODEL,
                        messages=chat_log,
                        max_tokens=1024,
        ...
        token_tracker.log_api_error(e)  # Track API errors
        ...
        token_tracker.log_error(e)  # Track general errors
        ...
        # Always end the interaction tracking
        token_tracker.end_interaction()
...
async def main():
    ...
    try:
        ...
        try:
            ...
            verification_status = await token_tracker.verify_setup()
            if verification_status.success:
                print(f"✓ Token Tracker: Initialized")
                print(f"✓ Model: {ANTHROPIC_MODEL}")
                print(f"✓ System Prompt Tokens: {verification_status.prompt_tokens}")
                print(f"✓ Client Connection: Active")
            else:
                print(f"{Fore.RED}Token tracking verification failed: {verification_status.error}{Fore.WHITE}")
        except Exception as e:
...
async def run_main_loop():
    """
    Main interaction loop handling the voice assistant's core conversation flow with integrated token management.
    ...
    Token Management:
    - Conversation sessions tracked from wake to completion
    - Each API interaction logged and monitored
    - Token limits enforced with graceful degradation
    """
    while True:
        try:
            ...
            if transcript:
                ...
                token_tracker.start_conversation_session()
                try:
                    res = await generate_response(transcript)
                    token_tracker.log_api_interaction(
                            ...
                            if follow_up:
                                ...
                                try:
                                    ...
                                    if "?" not in res:
                                        ...
                                        token_tracker.end_conversation_session()
                                        break
                                    ...
                                    token_tracker.log_error(str(api_error))
                                    await display_manager.update_display('idle')
                                    token_tracker.end_conversation_session()
                                    break
                            else:
