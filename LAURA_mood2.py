...
async def generate_response(query):
    ...
    try:
        ...
        if response.stop_reason == "tool_use":
            ...
            await generate_voice(random.choice(TOOL_SENTENCES))
...
async def wake_word():
    """Reliable wake word detection that correctly returns and doesn't get stuck"""
    try:
        ...
        try:
            ...
            # Main detection loop - split into small chunks with explicit sleeps
            max_listening_time = 0.5  # Check for half a second at a time
            ...
            while (time.time() - start_time) < max_listening_time:
                ...
                # If wake word detected, return True immediately
                if keyword_index >= 0:
            ...
            # No wake word detected in this time chunk
            return False
        ...
        traceback.print_exc()
        await asyncio.sleep(10)  # Sleep on error to avoid tight error loops
        return False
...
async def check_upcoming_events():
    """Improved notification system with direct audio manager signaling"""
    ...
    while True:
        try:
            ...
            for event in events:
                ...
                # Calculate minutes with proper rounding for better accuracy
                seconds_until = (start_time - now).total_seconds()
                minutes_until = round(seconds_until / 60)

                if DEBUG_CALENDAR:
                    print(f"DEBUG: Event '{summary}' starts in {minutes_until} minutes")

                # Check each notification interval
                for interval in CALENDAR_NOTIFICATION_INTERVALS:
                    notification_key = f"{event_id}_{interval}"
                    
                    # Only notify if we're close to the interval and haven't notified yet
                    if abs(minutes_until - interval) <= 2 and notification_key not in calendar_notified_events:
                        # Mark as notified BEFORE processing to prevent duplicates
                        calendar_notified_events.add(notification_key)
                        
                        if DEBUG_CALENDAR:
                            print(f"DEBUG: Need to notify about '{summary}' at {interval} minute mark")
                        
                        # Format the notification message with exact interval for accuracy
                        notification_text = random.choice(CALENDAR_NOTIFICATION_SENTENCES).format(
                            minutes=interval,
                            event=summary
                        )
                        
                        # Store current display state to restore later
                        previous_state = display_manager.current_state
                        previous_mood = display_manager.current_mood
                        
                        try:
                            print(f"NOTIFICATION: Playing calendar alert for {summary}")
                            
                            # Wait for any current audio to complete
                            await audio_manager.wait_for_audio_completion()
                            
                            # Update display for notification
                            await display_manager.update_display('speaking')
                            
                            # Generate notification audio
                            audio = tts_handler.generate_audio(notification_text)
                            
                            # Save to a notification-specific file
                            with open("notification.mp3", "wb") as f:
                                f.write(audio)
                            
                            # Play the notification with proper await
                ...
                # Clean up old notifications for past events
                if minutes_until < -5:  # Event is over 5 minutes ago
                    for interval in CALENDAR_NOTIFICATION_INTERVALS:
                        key = f"{event_id}_{interval}"
                        if key in calendar_notified_events:
                            calendar_notified_events.remove(key)
                            if DEBUG_CALENDAR:
                                print(f"DEBUG: Removed old notification key '{key}'")
...
async def main():
    ...
    try:
        ...
        # Calendar notification task (only one)
        calendar_check_task = asyncio.create_task(check_upcoming_events())
        ...
        for task in [rotate_task, audio_task, heartbeat_task, main_loop_task, calendar_check_task]:
            if not task.done():
                task.cancel()
                try:
                    await task
    ...
    while True:
        try:
            ...
            if display_manager.current_state in ['sleep', 'idle']:
                # Poll for wake word - returns quickly
                wake_detected = await wake_word()
                
                # Continue polling if no wake word
                if not wake_detected:
                    # Brief sleep to prevent CPU hogging
                    await asyncio.sleep(0.1)
                    continue
                
                # Wake word detected, proceed with activation
                if display_manager.current_state == 'sleep':
