...
from token_manager_v22 import TokenManager
...
async def generate_response(query):
    ...
    try:
        ...
        if use_tools:
            _, relevant_tools = token_tracker.get_tools_for_query(query)

        print(f"Debug - Tools Status: use_tools={use_tools}, relevant_tools={relevant_tools}")
        ...
        try:
            ...
            # Token tracking for initial response
            try:
                input_count = token_tracker.count_message_tokens(chat_log)
                output_count = token_tracker.count_message_tokens(response.content)
                ...
                print(f"DEBUG: Token counts - input: {input_count}, output: {output_count}")
        ...
        if response.stop_reason == "tool_use":
            ...
            if tool_results:
                ...
                try:
                    ...
                    try:
                        # Get token counts for the last two messages (tool-related)
                        ...
                        tool_output_count = token_tracker.count_message_tokens(final_response.content)
                        
                        # Update costs for tool usage
                        token_tracker.update_session_costs(tool_input_count, tool_output_count, True)
...
async def main():
    ...
    try:
        ...
        print(f"✓ Tool Status: {'Enabled' if token_tracker.tools_are_active() else 'Disabled'}")
        ...
        try:
            if display_manager.current_state in ['sleep', 'idle']:
                ...
                try:
                    ...
                    mood_match = re.match(r'\[(.*?)\](.*)', res)  # Modified pattern to capture both mood and remaining text
                    if mood_match:
                        ...
                        if len(groups) == 2:
                            ...
                            # Fallback if we don't get both parts
                            await display_manager.update_display('speaking')
