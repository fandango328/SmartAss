...
def trim_chat_log(log, max_tokens=None):
    """Trim the chat log to stay under token limits."""
    if not max_tokens:
    ...
    # Keep track of tool IDs to make sure we don't break tool pairs
    # Scan from the end, as recent tools are more important
    tool_ids_to_keep = set()
    
    # First pass - identify recent tool IDs (from last 10 messages)
    for message in log[-10:]:
        if isinstance(message.get("content"), list):
            for item in message.get("content", []):
                if isinstance(item, dict):
                    if item.get("type") == "tool_use":
                        tool_ids_to_keep.add(item.get("id"))
                    elif item.get("type") == "tool_result":
                        tool_ids_to_keep.add(item.get("tool_use_id"))
    
    # Start with a base of the last few messages which we'll always keep
    preserved_messages = log[-3:]
    remaining_messages = log[:-3]
    
    # Add any messages with tools we need to preserve
    tool_messages = []
    non_tool_messages = []
    
    for message in remaining_messages:
        has_important_tool = False
        
        if isinstance(message.get("content"), list):
            for item in message.get("content", []):
                if isinstance(item, dict):
                    if (item.get("type") == "tool_use" and item.get("id") in tool_ids_to_keep) or \
                       (item.get("type") == "tool_result" and item.get("tool_use_id") in tool_ids_to_keep):
                        has_important_tool = True
                        break
        
        if has_important_tool:
            tool_messages.append(message)
        else:
            non_tool_messages.append(message)
    
    # Add the important tool messages first
    result = []
    if system_message:
        result.append(system_message)
    
    # Start adding messages until we approach the token limit
    result.extend(tool_messages)  # Important tool messages first
    
    # Then add as many non-tool messages as will fit, starting from most recent
    non_tool_messages.reverse()  # Most recent first
    
    for message in non_tool_messages:
        # Add message
        test_log = result + [message] + preserved_messages
        test_tokens = token_tracker.count_message_tokens(test_log) if token_tracker else 0
    ...
    # Ensure our sanitize function is called as a final check
...
def sanitize_tool_interactions(chat_history):
    ...
    # Find all valid tool_use IDs
    valid_tool_ids = set()
    for message in chat_history:
        if message["role"] == "assistant" and isinstance(message["content"], list):
            for item in message["content"]:
                if isinstance(item, dict) and item.get("type") == "tool_use":
                    valid_tool_ids.add(item.get("id"))
    
    # Remove any tool_result that doesn't have a matching tool_use
    sanitized_history = []
    for message in chat_history:
        if message["role"] == "user" and isinstance(message["content"], list):
            # This might be a message with tool_results
            valid_content = []
            has_invalid = False
            
            for item in message["content"]:
                if isinstance(item, dict) and item.get("type") == "tool_result":
                    # Only keep valid tool_results
            ...
            if valid_content:
                ...
                sanitized_message["content"] = valid_content
                sanitized_history.append(sanitized_message)
            
            if has_invalid:
                print("DEBUG: Removed invalid tool_result entries")
