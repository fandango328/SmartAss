...
class TokenTracker:
    def __init__(self, anthropic_client):
        ...
        # Remove cost tracking since we'll handle that separately
        self.TOOL_OVERHEAD = 346

    def _initialize_system_prompt_count(self) -> None:
        """Don't make an API call just to count tokens"""
        if SYSTEM_PROMPT:
            self.session_tokens["input"]["system_prompt"] = len(SYSTEM_PROMPT.split())  # Rough estimate
            self.session_tokens["input"]["total"] += self.session_tokens["input"]["system_prompt"]

    def update_query_tokens(self, query: str, chat_history: List[Dict[str, str]], using_tools: bool = False) -> None:
        # Simple word-based counting instead of API calls
        query_tokens = len(query.split())
        history_tokens = sum(len(msg["content"].split()) for msg in chat_history) if chat_history else 0
        ...
        self.session_tokens["input"]["total"] += query_tokens + history_tokens + tool_overhead
    ...
    def update_response_tokens(self, response_content: Union[str, List[Dict]], is_tool_result: bool = False) -> None:
        if isinstance(response_content, str):
            ...
            tokens = len(json.dumps(response_content).split())
    ...
    def print_metrics(self, label: str = "") -> None:
        """Print token usage metrics without cost calculations"""
        print(f"\n=== Token Metrics {label} ===")
        print("Input Tokens:")
        ...
        print(f"\nTotal Session Tokens: {self.session_tokens['input']['total'] + self.session_tokens['output']['total']}")
...
async def generate_response(query):
    ...
    try:
        # Simple check if query might need tools
        needs_tools = any(keyword in query.lower() for keyword in [
            "email", "calendar", "schedule", "event", "task", 
            "reminder", "contact", "location", "time", "date", 
            "calibrate"
        ])
