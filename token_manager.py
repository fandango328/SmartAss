...
class TokenManager:
    ...
    def count_message_tokens(self, current_query: str, chat_log: list = None, system_prompt: str = None) -> int:
        try:
            # Check if there are any images in the chat log
            if chat_log and isinstance(chat_log, list):
                for msg in chat_log:
                    if isinstance(msg.get('content'), list):
                        # If any content item is an image, skip token counting
                        if any(isinstance(item, dict) and item.get('type') == 'image' 
                              for item in msg['content']):
                            print("DEBUG: Skipping token count for message containing image")
                            return 0  # Return 0 tokens for messages with images
