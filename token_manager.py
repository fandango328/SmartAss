    ...
    def handle_tool_command(self, query: str) -> Tuple[bool, Dict[str, Any]]:
        try:
            ...
            if has_enable_command and has_disable_command:
                ...
                return True, self._handle_disable_command() if last_disable_pos > last_enable_pos else self._handle_enable_command()
                
            elif has_disable_command:
                return True, self._handle_disable_command()
            elif has_enable_command:
                return True, self._handle_enable_command()
    ...
    def _handle_enable_command(self) -> Dict[str, Any]:
        try:
            if not self.tools_enabled:
                self.tools_enabled = True
                # Play enable audio
                audio_file = random.choice(os.listdir(SOUND_PATHS['tool']['status']['enabled']))
                AudioManager.play_audio(os.path.join(SOUND_PATHS['tool']['status']['enabled'], audio_file))
                return {"success": True, "state": "enabled", "mood": "happy"}
            
            return {"success": True, "state": "enabled", "mood": "casual"}
            
        except Exception as e:
            return {"success": False, "state": "error", "mood": "disappointed"}
    ...
    def _handle_disable_command(self) -> Dict[str, Any]:
        try:
            if self.tools_enabled:
                self.tools_enabled = False
                # Play disable audio
                audio_file = random.choice(os.listdir(SOUND_PATHS['tool']['status']['disabled']))
                AudioManager.play_audio(os.path.join(SOUND_PATHS['tool']['status']['disabled'], audio_file))
                return {"success": True, "state": "disabled", "mood": "casual"}
