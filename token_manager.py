...
class TokenManager:
    ...
    def count_message_tokens(self, current_query: str, chat_log: list = None, system_prompt: str = None) -> tuple[int, int]:
        """Count tokens using Anthropic's official endpoint while maintaining existing tracking."""
        try:
            # Use the existing prepare_messages_for_token_count to format properly
    ...
    def count_output_tokens(self, response_text: str) -> int:
        ...
        """
        # Initial simple implementation - can be refined based on testing
        char_count = len(response_text)
        estimated_tokens = char_count // 4
        
        # Log for verification against actual API costs
